...
// The "ready client prefix" is the first `prefix_limit` (= per_upstream_min_connections) elements
// of `ready_clients_`, over which new streams are spread round-robin. `prefix_tail` points at the
...
ConnectionPool::Cancellable* ConnPoolImplBase::newStreamImpl(AttachContext& context,
  ...
  if (!ready_clients_.empty()) {
    ...
    rotateReadyPrefix(ready_clients_, ready_prefix_tail_,
                      host_->cluster().perUpstreamMinConnections());
...
void ConnPoolImplBase::transitionActiveClientState(ActiveClient& client,
  ...
  const uint32_t prefix_limit = host_->cluster().perUpstreamMinConnections();
...
void ConnPoolImplBase::onConnectionEvent(ActiveClient& client, absl::string_view failure_reason,
  ...
  case Network::ConnectionEvent::LocalClose: {
    ...
    removeFromReadyPrefix(ready_clients_, ready_prefix_tail_,
                          host_->cluster().perUpstreamMinConnections(), client);
