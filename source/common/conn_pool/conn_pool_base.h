...
public:
  ...
  // per_upstream_min_connections ready clients). Maintained by the pool as ready clients are added
  // and removed; only meaningful while the client is in `ready_clients_`. See
...
protected:
  ...
  // The last client of the ready-client prefix -- the first per_upstream_min_connections ready
  // clients, over which new streams are spread round-robin. `nullptr` when the prefix is empty or
  // the floor is 0 (round-robin disabled). Maintained as clients enter and leave `ready_clients_`;
  // see the prefix helpers in the .cc.
