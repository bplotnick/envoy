...
// With a per_upstream_min_connections floor, streams round-robin over the ready-client prefix
// instead of piling onto the front one (the LIFO default). Two connections are kept ready via a
// high concurrency limit; repeated attaches spread evenly between them. The debug invariant check
// for the ready-client prefix runs on every add and rotation.
TEST_F(ConnPoolImplBaseTest, RoundRobinOverReadyPrefix) {
  ...
  ON_CALL(*cluster_, perUpstreamMinConnections()).WillByDefault(Return(2));

  // The floor primes two connections on the first request; connect both so both are Ready.
  pool_.newStreamImpl(context_, /*can_send_early_data=*/false);
